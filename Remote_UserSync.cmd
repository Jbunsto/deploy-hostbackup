...
IF NOT EXIST \\%COMPUTER%\c$ ECHO %COMPUTER% - No access, skipping
...
FOR /f %%U in ('dir /b /A:D %BACKUPDRIVELETTER%:\%BACKUPROOT%\%COMPUTER%\USMT\File\C$\Users') do (
	...
	if !Flag!==good (
		...
		For /f %%F in (path_users_folders.txt) do (					
			...
			robocopy "!source!\%%F" "!destination!\%%F" /s /XO /FFT /R:1 /W:2 /SL													
		)				
				
		REM Robocopy Documents folder.
		robocopy "!source!\Documents" "!destination!\Documents" /s /XO /FFT /R:1 /W:2 /SL							
	)			
)
